...
def trim_filename(name: str) -> str:
    """ファイル名の前後にある空白・書式文字・制御文字を切り詰める。

    str.strip() は半角/全角スペースや NBSP は除去するが、
    ゼロ幅スペース(U+200B)や BOM(U+FEFF) などの書式文字は残してしまう。
    それらも含めて除去することで、空白始まりのファイル名を防ぐ。
    """
    def is_trim(ch: str) -> bool:
        if ch.isspace():
            return True
        # Cc=制御, Cf=書式(ゼロ幅スペース/BOM等), Zs/Zl/Zp=区切り
        return unicodedata.category(ch) in ("Cc", "Cf", "Zs", "Zl", "Zp")

    start, end = 0, len(name)
    while start < end and is_trim(name[start]):
        start += 1
    while end > start and is_trim(name[end - 1]):
        end -= 1
...
def main():
    ...
    # ファイル名前後の空白・書式文字を切り詰める（空白始まり/終わりのファイル名を防ぐ）
    output_prefix = trim_filename(output_prefix)
    if not output_prefix:
        output_prefix = playlist_id
